...
sudo -E apt-get -qq update && sudo apt-get upgrade && sudo apt-get install ca-certificates
if [ ! -x "$(command -v curl)" ]; then
    echo "curl not found, installing..."
    sudo -E apt-get -qq install curl
fi
packages_file="${BUILDER_TMP_DIR}/packages.txt"
packages_url="https://github.com/Kastilo/openwrt-buildenv/blob/master/packages.txt"
(
  set +eo pipefail
